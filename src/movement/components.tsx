...
import {
  AlertTriangle, CheckCircle2, Clock, ClipboardCopy, Flame, Lock, MessageSquare,
  Phone, Play, RefreshCw, Sparkles, Timer, TrendingDown, Unlock, Users, Zap,
} from "lucide-react";
...
import { DRAFT_META, NEXT_ACTION_LABEL, PRIORITY_LABEL, type CallResult, type DraftCode, type MovementState } from "./types";
import { suggestAfterCall, type SmartSuggestion } from "@/lib/smart-next-action";
...
function ActiveRow({ n, r, meta, active, onSelect }: {
  ...
  const isOverdue = r.state.nextAction && new Date(r.state.nextAction.dueAt).getTime() < Date.now();
  return (
        ...
        <div className="text-[11px] text-muted-foreground truncate">
          ...
          {r.state.nextAction && (
            <> · <span className={cn(isOverdue && "text-destructive font-medium")}>
              {NEXT_ACTION_LABEL[r.state.nextAction.kind]} · {new Date(r.state.nextAction.dueAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </span></>
          )}
        </div>
        <div className="text-[10px] text-muted-foreground">
          Owner: {r.state.primaryOwnerName}
          {r.state.nextAction?.ownerName && r.state.nextAction.ownerName !== r.state.primaryOwnerName && ` → ${r.state.nextAction.ownerName}`}
        </div>
      </div>
...
export function WorkPanel({ ulid, meta }: { ulid: string | null; meta: Meta }) {
  ...
  const [lastCallResult, setLastCallResult] = useState<CallResult | null>(null);
  ...
  const held = !!lock;
  const isOverdue = st.nextAction && new Date(st.nextAction.dueAt).getTime() < Date.now();

  function handleCall(result: CallResult) {
    mv.logCall(ulid!, result, note);
    setLastCallResult(result);

    // Smart Next Action: auto-suggest after call
    const noAnswerStreak = result === "no-answer" || result === "busy"
      ? (mv.events.filter((e) => e.ulid === ulid && e.kind === "call" && e.text?.includes("no-answer")).length + 1)
      : 0;
    const sug = suggestAfterCall(st!, result, noAnswerStreak);
    setSuggestion(sug);
    ...
    audit.log({
      ...
      entityId: ulid!,
      action: "CALL_LOGGED",
      before: { stage: st!.stage },
      after: { callResult: result },
      summary: `${mv.actor.name} logged call: ${result} for ${st!.name ?? ulid}`,
    });
    ...
    void pushWorkflowEvent({
      ...
      action: "CALL_LOGGED",
      module: "movement-os",
      afterState: { result, stage: st!.stage },
      summary: `${mv.actor.name} logged call: ${result} for ${st!.name ?? ulid}`,
    });

    toast.success(`Call logged: ${result} — smart suggestion ready ↓`);
  }

  function acceptSuggestion() {
    if (!suggestion) return;
    const dueAt = new Date(Date.now() + suggestion.dueInMins * 60_000).toISOString();
    ...
    // Persist next action to Supabase
    ...
    audit.log({
      ...
      after: { kind: suggestion.kind, dueAt, source: "smart-engine" },
      summary: `${mv.actor.name} accepted smart suggestion: ${suggestion.label} for ${st!.name ?? ulid}`,
    });

    toast.success(`Next action accepted: ${suggestion.label}`);
    setSuggestion(null);
  ...
  async function copyMessage() {
    ...
    mv.sendMessage(ulid!, generatedMsg);
  ...
  return (
    <div className={cn("rounded-lg border bg-card p-4 space-y-4", isOverdue ? "border-destructive/50" : "border-border")}>
      {/* ── Outcome statement ── */}
      <div className="text-[10px] text-muted-foreground bg-muted/50 rounded px-2 py-1">
        <Sparkles className="h-3 w-3 inline mr-1" />
        Result: Every action here ends with a recorded outcome, next step, and auto-generated follow-up message.
      </div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {info?.phone} · {info?.area} · owner <strong>{st.primaryOwnerName}</strong>
          </div>
          {isOverdue && st.nextAction && (
            <div className="text-xs text-destructive font-semibold flex items-center gap-1 mt-1">
              <AlertTriangle className="h-3.5 w-3.5" />
              OVERDUE: {NEXT_ACTION_LABEL[st.nextAction.kind]} was due {rel(st.nextAction.dueAt)} ago
            </div>
          )}
        </div>
        {held ? (
        ...
        <Cell label="Next action">
          {st.nextAction ? (
            <span className={cn(isOverdue && "text-destructive font-semibold")}>
              {NEXT_ACTION_LABEL[st.nextAction.kind]} · {new Date(st.nextAction.dueAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </span>
          ) : "—"}
        </Cell>
        <Cell label="Owner">{st.nextAction?.ownerName ?? st.primaryOwnerName}</Cell>
        <Cell label="Deadline">
          {st.nextAction ? (
            <span className={cn(isOverdue && "text-destructive font-semibold")}>
              {new Date(st.nextAction.dueAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              {isOverdue && " ⚠️"}
            </span>
          ) : "—"}
        </Cell>
      ...
      <div className="space-y-2">
        <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Call · click result → get smart suggestion</div>
        <div className="flex flex-wrap gap-1.5">
          {(["connected", "no-answer", "busy", "wrong-number", "rejected"] as const).map((r) => (
            <Button key={r} size="sm" variant={lastCallResult === r ? "default" : "outline"} className="h-7 text-[11px]"
      ...
      {/* ── SMART NEXT ACTION BAR (appears after call) ── */}
      {suggestion && (
        <div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-3 space-y-3">
          <div className="flex items-center gap-2">
            <Zap className="h-4 w-4 text-primary" />
            <span className="text-xs font-semibold text-primary">Smart Next Action</span>
            <Badge variant="outline" className="text-[10px] ml-auto">{suggestion.reason}</Badge>
          </div>
          <div className="grid grid-cols-3 gap-2 text-[11px]">
            <Cell label="Action">{NEXT_ACTION_LABEL[suggestion.kind]}</Cell>
            <Cell label="Due in">{suggestion.dueInMins < 60 ? `${suggestion.dueInMins}m` : `${Math.round(suggestion.dueInMins / 60)}h`}</Cell>
            <Cell label="Owner">{st.primaryOwnerName}</Cell>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Textarea
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && e.metaKey) { e.preventDefault(); copyMessage(); } }}
            />
          </div>

          <div className="flex gap-2">
            <Button size="sm" onClick={acceptSuggestion} className="flex-1">
              <CheckCircle2 className="h-3.5 w-3.5 mr-1" /> Accept & Set Next Action
            </Button>
            <Button size="sm" variant="outline" onClick={copyMessage}>
              <ClipboardCopy className="h-3.5 w-3.5 mr-1" /> Copy Message
            </Button>
            <Button size="sm" variant="ghost" onClick={() => { setSuggestion(null); setLastCallResult(null); }}>
              Skip
            </Button>
          </div>
        </div>
      ...
      <div className="space-y-2">
        ...
        <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note for the next step…" className="h-8 text-xs"
          onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); nextIn(120, "call"); } }} />
        <div className="flex flex-wrap gap-1.5">
