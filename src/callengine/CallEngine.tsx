...
import { pushWorkflowEvent } from "@/lib/workflow-sync";
...
export function CallEngine({ lead, onLogged }: Props) {
  ...
  function commit() {
    ...
    // Persist workflow event to Supabase
    void pushWorkflowEvent({
      customerId: lead.ulid,
      canonicalId: lead.canonicalId,
      ...
      module: "call-engine",
      afterState: { outcome, movement: outputs.movement, nextStep: outputs.nextStep.kind },
      summary: `${mv.actor.name} committed call: ${def.label} · ${MOVEMENT_LABEL[outputs.movement]} · next: ${outputs.nextStep.label} for ${lead.name ?? lead.ulid}`,
    });
    ...
    audit.log({
      ...
      entityId: lead.ulid,
      action: "CALL_COMMITTED",
      after: { outcome, movement: outputs.movement, nextStep: outputs.nextStep.kind },
      summary: `${mv.actor.name} committed M-POWER CALL: ${def.label} for ${lead.name ?? lead.ulid}`,
    });
  ...
  return (
    <div className="space-y-3 rounded-lg border p-3">
      ...
      <div className="sticky top-0 z-20 -mx-3 -mt-3 mb-1 space-y-1.5 border-b bg-background/95 px-3 py-2 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        {/* Outcome statement */}
        <div className="text-[9px] text-muted-foreground bg-primary/5 rounded px-2 py-0.5 border border-primary/20">
          ✨ Result: Every completed call ends with a recorded outcome, auto-generated WhatsApp message, follow-up, and next step with an SLA.
        </div>
        <div className="flex items-center justify-between gap-2">
          ...
          {lead.nextAction && (
            <Badge variant="outline" className={cn("shrink-0 text-[10px]",
              new Date(lead.nextAction.dueAt).getTime() < Date.now() && "border-destructive text-destructive")}>
              next: {NEXT_ACTION_LABEL[lead.nextAction.kind]} ·{" "}
              {new Date(lead.nextAction.dueAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              {new Date(lead.nextAction.dueAt).getTime() < Date.now() && " ⚠️ OVERDUE"}
            </Badge>
          )}
        </div>
      ...
      {phase === "outputs" && outputs && (
        <>
          ...
          {/* Compact single-screen layout: message + follow-up + next step */}
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="space-y-1.5">
              <Title>1 · Send now (copy → WhatsApp)</Title>
              {outputs.mediaHint && <div className="text-[10px] text-muted-foreground">{outputs.mediaHint}</div>}
              <Textarea rows={5} className="text-xs" value={nowText} onChange={(e) => setNowText(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter" && e.metaKey) { e.preventDefault(); copy(nowText); } }} />
              <Button size="sm" variant="outline" className="w-full text-[11px]" onClick={() => copy(nowText)}>Copy for WhatsApp (⌘+Enter)</Button>
            </div>
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <div className="text-[10px] text-muted-foreground">{outputs.followUp.trigger} — auto-cancels if customer moves.</div>
                <Textarea rows={3} className="text-xs" value={followText} onChange={(e) => setFollowText(e.target.value)} />
              </div>
          ...
          <div className="flex gap-2">
            ...
            <Button className="flex-1" size="sm" onClick={commit}
              ...
              ✓ Commit: send message, arm follow-up, set next step
            </Button>
          </div>
        </>
      )}
